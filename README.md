...
The viewer opens `.dvi` files in a native macOS window. It renders DVI files directly with a built-in parser and AppKit renderer that handles pages, positioning, text, rules, zooming, TeX font metrics, Type 1 outline fonts, page-size specials, and common color specials. If the native parser cannot read a file, the app can still fall back to `dvipdfmx` and PDFKit when a TeX converter is installed.
